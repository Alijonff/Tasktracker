...
- Логика задач уровня UNIT изменена с «задача для управления» на «задача для отдела» (терминология выровнена под фактическое использование отделов как адресатов задач).
...
- Заместитель может создавать задачи для директора и любых сотрудников своего департамента, но не для себя.
...
- система автоматически назначает deadline_at как 18:00 следующего календарного дня (локальное время компании).

deadline_at задаёт момент основного дедлайна аукциона. Формальный момент закрытия аукциона = deadline_at + 3 часа.

4.2. Участие в аукционе. Участие в аукционе
...
4.3. Ставки (AuctionBid). Ставки (AuctionBid)
...
При создании задачи:
- deadline_at автоматически устанавливается на 18:00 следующего календарного дня;
- формальное закрытие аукциона происходит в deadline_at + 3 часа (то есть в 21:00 этого дня).

2) Период до deadline_at (рост цены)

- Начиная с первого контрольного момента после момента создания задачи (created_at), base_price растёт дискретно в эти контрольные моменты.
- К каждому последующему контрольному моменту коэффициент увеличения плавно движется от 1.0× к максимуму 1.5× так, чтобы к моменту deadline_at (18:00 следующего дня) цена достигала не более 1.5× base_price.
- Шаг увеличения между соседними контрольными моментами равномерно распределяется по доступным контрольным точкам до deadline_at.
- Рост работает **только пока по задаче нет ни одной активной ставки**.
- Как только появляется первая активная ставка, рост цены останавливается и текущее значение фиксируется.

3) Период после deadline_at (18:00–21:00)

- Формальный момент закрытия аукциона: deadline_at + 3 часа (обычно 21:00).
- В период от deadline_at до deadline_at + 3 часа участники могут продолжать делать ставки по стандартным правилам аукциона.
- В этот период дальнейший рост base_price не происходит, даже если до deadline_at ставок не было.
...
При создании задачи:
- deadline_at устанавливается на 18:00 следующего календарного дня;
- формальное закрытие аукциона происходит в deadline_at + 3 часа (обычно 21:00).

2) Период до deadline_at (рост времени)

- Начиная с первого контрольного момента после created_at, base_time_minutes растёт дискретно в указанные контрольные моменты.
