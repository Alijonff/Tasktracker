...
- id
- name
- department_id (управление всегда принадлежит какому-либо департаменту)
...
- могут создавать задачи и аукционы уровня отдела и департамента (UNIT/DEPARTMENT);
...
- Не управляют структурой департамента в целом (не создают/удаляют департаменты, управления, отделы).
- Могут иметь повышенный грейд и стартовые баллы.
- Могут участвовать в аукционах как исполнители (если не admin и удовлетворяют условиям области и min_grade).
- Не создают задачи и аукционы.

1.4. Обычные и старшие сотрудники

- Могут видеть задачи, доступные по их департаменту/управлению/отделу и грейду.
- Могут делать ставки в аукционах, если удовлетворяют условиям участия (область, min_grade, не admin, не создатель).
- Могут быть исполнителями задач.
- Не создают задачи, не управляют структурой, не меняют роли и не увольняют сотрудников.

1.5. Право создания задач

- Создавать задачи любых типов (INDIVIDUAL, UNIT, DEPARTMENT) и аукционы могут только директор департамента и его заместитель в рамках своего департамента.
- Директор и его заместитель не могут создавать задачи для самих себя (creator_id ≠ executor_id).
- **Важно:** Директору департамента НЕЛЬЗЯ назначить индивидуальную задачу (ни кем, даже заместителем). Директор может стать исполнителем только автоматически, если аукцион, созданный им, завершился без ставок.
- Директор может создавать задачи для заместителя и любых сотрудников своего департамента.
- Заместитель может создавать задачи для директора (ТОЛЬКО аукционы, где директор может стать исполнителем при отсутствии ставок) и любых сотрудников своего департамента, но не для себя.

1.6. Ограничения и валидация ролей

- При создании или обновлении задач система должна валидировать:
  - creator_id не может ссылаться на сотрудника с ролью admin;
  - executor_id не может ссылаться на сотрудника с ролью admin и должен принадлежать тому же департаменту, что и задача;
  - для задач типов UNIT и DEPARTMENT все участники аукциона (employee_id в AuctionBid) не могут иметь роль admin и должны удовлетворять условиям области и min_grade.
- При попытке нарушить эти правила API обязано возвращать ошибку валидации.
...
- Назначается конкретному сотруднику департамента (executor_id установлен при создании).
- Создатель задачи (creator_id) должен быть директором или заместителем директора этого департамента.
- Исполнитель (executor_id) должен:
  - принадлежать тому же департаменту;
  - быть активным (is_active = true);
  - не иметь роль admin;
  - не совпадать с creator_id.
- type = INDIVIDUAL, auction_mode = null.
- Статус при создании: IN_PROGRESS.
- Задачу видят создатель, исполнитель, а также руководители по стандартным правилам доступа.

2.2. UNIT (задача для отдела)

- type = UNIT.
- department_id и unit_id задаются обязательно (management_id используется, если отдел подчинён управлению).
- auction_mode ∈ {MONEY, TIME} (обязательно).
- Статус при создании: BACKLOG (открыт аукцион).
- Задачу видят все сотрудники департамента.
- Ставки делать могут только сотрудники **конкретного отдела**, указанного в unit_id, чей grade ≥ min_grade и чья роль НЕ admin (при наличии management_id у отдела сотрудник также должен принадлежать этому управлению).
...
- IN_PROGRESS → UNDER_REVIEW — исполнитель отправляет результат.
- UNDER_REVIEW → DONE — создатель принимает работу (или директор департамента/админ).
- UNDER_REVIEW → IN_PROGRESS — создатель возвращает на доработку (с комментарием).
...
- задаются: auction_mode, min_grade, область (department_id и при необходимости unit_id/management_id для задач UNIT);
- для MONEY — base_price (исходная цена);
- для TIME — base_time_minutes (исходное время в минутах);
- система автоматически назначает deadline_at как 18:00 следующего рабочего дня (пропуская выходные).

deadline_at задаёт момент основного дедлайна аукциона. Формальный момент закрытия аукциона = deadline_at + 3 часа.

4.2. Участие в аукционе

Условие для участия (ставки):
- сотрудник активен (is_active = true);
- принадлежит подходящей области:
  - для задач типа UNIT: тому же департаменту и тому же отделу (department_id и unit_id совпадают с задачей; если у отдела есть management_id, сотрудник должен быть в этом управлении);
  - для задач типа DEPARTMENT: тому же департаменту;
- его grade ≥ min_grade задачи;
- он не является создателем задачи;
- его роль НЕ admin (admin никогда не участник аукциона и не может быть исполнителем).

Условия видимости:
- задачей департамента может просматривать любой сотрудник этого департамента;
- задачи отдела (UNIT) видят все сотрудники департамента (прозрачность), но ставки могут делать только сотрудники конкретного отдела.
...
Аукцион в режиме MONEY считается полностью завершённым в момент закрытия аукциона.

- По умолчанию момент закрытия аукциона = deadline_at + 3 часа (расширенное окно после дедлайна).
- Если в дальнейшем будет введён другой механизм закрытия (например, ручное завершение), он должен быть явно зафиксирован в документации и реализован единообразно.

По состоянию на момент закрытия аукциона:
- выбирается минимальная активная ставка;
- при равенстве сумм выбирается сотрудник с большим current_points на момент закрытия;
- при полном равенстве побеждает ставка с более ранним created_at.
...
Рост базовой цены и поведение аукциона зависят от наличия ставок.

1) Контрольные моменты роста

Для роста используются фиксированные контрольные моменты времени в сутках:
- 00:00, 03:00, 06:00, 09:00, 12:00, 15:00, 18:00, 21:00.

При создании задачи:
- deadline_at автоматически устанавливается на 18:00 следующего рабочего дня (пропуская выходные);
- формальное закрытие аукциона происходит в deadline_at + 3 часа (то есть в 21:00 этого дня).

2) Период до deadline_at (рост цены)

- Начиная со **второго** контрольного момента после created_at, base_price растёт дискретно в указанные контрольные моменты.
- К каждому следующему контрольному моменту коэффициент увеличения движется от 1.0× к максимуму 1.5× к моменту deadline_at.
- Рост работает **только пока по задаче нет ни одной активной ставки**.
- Как только появляется первая активная ставка, рост базовой цены останавливается и значение фиксируется.

4) Полное отсутствие ставок

- Если к моменту закрытия аукциона (deadline_at + 3 часа) по задаче нет ни одной активной ставки:
  - задача автоматически назначается создателю (executor_id = creator_id);
  - статус задачи → IN_PROGRESS;
  - итоговая цена = фактическое значение base_price с учётом роста (обычно 1.5× или меньше, если доступных контрольных точек было мало);
  - TaskResult считается так, как если бы создатель выиграл аукцион с этой ценой.

5.4. Начисление вознаграждения (MONEY) Начисление вознаграждения (MONEY)

При переводе задачи в DONE:
- рассчитываются базовые баллы задачи исходя из минимального грейда (min_grade):
  - Grade D: 3 балла
  - Grade C: 6 баллов
  - Grade B: 9 баллов
  - Grade A: 12 баллов
...
Аукцион в режиме TIME считается полностью завершённым в момент закрытия аукциона.

- По умолчанию момент закрытия аукциона = deadline_at + 3 часа (расширенное окно после дедлайна).

По состоянию на момент закрытия аукциона:
- выбирается минимальное value_time_minutes;
- при равенстве — сотрудник с большим current_points;
- при полном равенстве — ставка с более ранним created_at.
...
Если до момента deadline_at по задаче нет активных ставок:

1) Контрольные моменты роста

1) Контрольные моменты роста

Для роста базового времени используются те же контрольные моменты, что и для денег:
- 00:00, 03:00, 06:00, 09:00, 12:00, 15:00, 18:00, 21:00.

При создании задачи:
- deadline_at устанавливается на 18:00 следующего рабочего дня (пропуская выходные);
- формальное закрытие аукциона происходит в deadline_at + 3 часа (обычно 21:00).

2) Период до deadline_at (рост времени)

- Начиная со **второго** контрольного момента после created_at, base_time_minutes растёт дискретно в указанные контрольные моменты.
  - *Пример:* Если аукцион создан в 05:00, следующий контрольный момент 06:00 (пропускается), рост начинается с 09:00.
- К каждому следующему контрольному моменту коэффициент увеличения движется от 1.0× к максимуму 1.5× к моменту deadline_at.
- Рост работает **только пока по задаче нет ни одной активной ставки**.
- Как только появляется первая активная ставка, рост базового времени останавливается и значение фиксируется.

3) Период после deadline_at (18:00–21:00)

- Формальный момент закрытия аукциона: deadline_at + 3 часа.
- В период от deadline_at до deadline_at + 3 часа участники могут продолжать делать ставки.
- В этот период рост base_time_minutes больше не происходит.
...
- Если к моменту закрытия аукциона (deadline_at + 3 часа) по задаче нет ни одной активной ставки:
  ...
  - статус задачи → IN_PROGRESS;
  - итоговое базовое время = фактическое значение base_time_minutes с учётом роста;
  - TaskResult считается так, как если бы создатель выиграл аукцион с этим временем.

6.5. Начисление вознаграждения (TIME) Начисление вознаграждения (TIME)
...
- за каждый час просрочки в рабочее время начисляется −1 балл (штраф).
  ...
  - Часовой пояс: Asia/Tashkent (Узбекистан).
  - Праздники пока считаются рабочими днями (если выпадают на будни), выходные (суббота, воскресенье) не учитываются.
...
- сотрудник остаётся участником всех активных аукционов этого департамента;
- активные ставки по задачам типов UNIT и DEPARTMENT не аннулируются, пока сотрудник числится в том же департаменте.
...
- Основная цель отчётности — показать эффективность работы сотрудников и их вклад на уровне отдела, управления и департамента.
...
Отчётность строится по иерархии:
1) **Сотрудник** — самый нижний уровень.
2) **Отдел** — агрегирует результаты сотрудников этого отдела.
3) **Управление** — агрегирует результаты сотрудников и отделов, входящих в управление.
4) **Департамент** — агрегирует результаты всех управлений и сотрудников департамента.

Правило учёта руководителей:
- Баллы директора управления и его заместителя учитываются в управлении, которым они руководят.
- Баллы директора департамента и его заместителя учитываются в департаменте.
