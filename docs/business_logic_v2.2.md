...
2.2. UNIT (задача для управления) (задача для управления)

- type = UNIT.
- department_id и management_id задаются обязательно.
- auction_mode ∈ {MONEY, TIME} (обязательно).
- Статус при создании: BACKLOG (открыт аукцион).
- Задачу видят все сотрудники департамента.
- Ставки делать могут только сотрудники **конкретного управления**, указанного в management_id, чей grade ≥ min_grade и чья роль НЕ admin.
...
- задаются: auction_mode, min_grade, область (department_id и при необходимости management_id);
...
- принадлежит подходящей области:
  - для задач типа UNIT: тому же департаменту и тому же управлению (department_id и management_id совпадают с задачей);
